 ...
 * Adding a LLM Param:
 *
 *  - Step 1. add label to [GptToolBundle] with key `settings.<yourName>`
 ...
 *
 *
 * @param label the label of the setting, will automatically get from bundle resource [GptToolBundle], named by `settings.${property.name}`
 * @param value the value of the setting, default from bundle resource [GptToolBundle], named `settings.${property.name}.default`
...
class LLMParam(
    ...
    companion object {
//        private val bundle = GptToolBundle

        /**
         * @param block a block to create a LLMParam, will be called only once
         *
         * will set [label] and [value] to the value defined in config file [GptToolBundle] if they are empty
         */
                    ...
                    override fun getValue(thisRef: Any?, property: KProperty<*>): LLMParam {
                        return param ?: this@Companion.block().apply {
                            if (label.isEmpty()) {
                                ...
                                label = runCatching {  property.name }.getOrElse {
                                    "WARNNING-KEY:add key: settings.$key to GptToolBundle.properties"
                                }
                            }
